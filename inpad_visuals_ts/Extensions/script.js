...
document.getElementById('contrBtn').addEventListener('click', function() {
	areaMode = !areaMode;
	if (areaMode) {
		this.textContent = 'Выйти из режима выделения';
		this.style.backgroundColor = '#90EE90';
	} else {
		this.textContent = 'Выделить территорию';
...
document.getElementById('button1').addEventListener('click', () => {
	nameObj = 'scene_P.gltf';
});

document.getElementById('button2').addEventListener('click', () => {
	nameObj = 'scene_M.gltf';
});

document.getElementById('button3').addEventListener('click', () => {
	nameObj = 'scene_3.gltf';
});
